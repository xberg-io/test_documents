...
A lightweight toolkit for adding metrics, traces, and structured logs to
a service without wiring three separate libraries. It exposes one
initializer and one exporter, so instrumentation stays out of your
business logic.
...
The init call reads its exporter target from OBSERV_ENDPOINT and falls
back to standard output when the variable is unset.
